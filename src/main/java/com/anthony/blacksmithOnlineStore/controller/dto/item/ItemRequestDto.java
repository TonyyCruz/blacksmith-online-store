...
public record ItemRequestDto(
    ...
    @Min(value = 0, message = "Base damage cannot be less than 0") Integer baseDamage,
    @Min(value = 0, message = "Base defense cannot be less than 0") Integer baseDefense,
    @Min(value = 0, message = "Base price must not be a negative number") BigDecimal basePrice,
    @Min(value = 0, message = "Final price must not be a negative number") BigDecimal finalPrice,
    @Size(min = 10, message = "Description must have at lest 10 characters") String description,
    @Min(value = 0, message = "Weight price must not be a negative number") Double weight,
    @Min(value = 0, message = "Stock price must not be a negative number") Integer stock,
